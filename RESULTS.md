...
| S2 | MCP 协议层 + 工具真实可用（无 LLM） | ✅ **8/8 PASS** | 19 个工具；背包 `0 → 2` 真实改变世界 |
...
MCP SDK 要求**一个 `McpServer` 实例只能连一个传输**，而 HTTP 模式下 DSH 每次运行都会新建会话，所以把 16 个工具的注册包成了 `buildServer()` 工厂：每个会话一个 `McpServer`，但**共用同一个 `BotManager`**（也就是同一个 Minecraft 连接）。改造后 stdio 路径经回归测试确认未被破坏（R1 8/8）。
